function [Data_use,judg,Guard_atertimelock]=TimeLockFun(PN,datain,lengh_data,m)
%%%%[数据帧包，时间同步是否成功(1:成功，0失败)]=TimeLockFun（PN序列，输入数据，数据帧长度）
%% 时间同步
%% 参数设置
K=720/log2(m);%每个pn序列的长度
Npn=4;%%一共N段pn序列
...
min_loc=min(true_loc);
true_loc=[min_loc,min_loc+K,min_loc+2*K,min_loc+3*K];
...
if (length(true_loc)<3)
    ...
    if (true_loc(2)+(Npn-fix(true_loc(2)/K))*K+lengh_data-1)>size(datain,1)
        Data_use = ones(lengh_data,1);
    else
