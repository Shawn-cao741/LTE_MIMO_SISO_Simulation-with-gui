function  MIMOfuncLTE(timedelay,frequbias,mode,EbNo)
...
%参数设置
stp=14;%%%%导频间隔：14=>15
N_subcarrier=4096;%子载波数128,1024
Npn=4;%%一共N段pn序列
global K

k = log2(2);
 snr = EbNo + 10*log10(k) + 10*log10(1/2);

Num_frame=14;
kk=201:stp+1:N_subcarrier-200;%(stp-1)/2:stp+1:1024;%当前间隔15
kkk=[kk,kk+1];
kongkou=timedelay*1e-6;
addzero=ceil((kongkou/2.5e-6)*100);
JIA0_NUM=41;
profile="EVA 5Hz";
if(mode~=1)
    mode=mode-1;
...
mod = comm.OFDMModulator('NumGuardBandCarriers',[200;N_subcarrier-kk(end)-1],...
...
BER_zui=[];BER_zf11=[];BER_zf21=[];BER_zf12=[];BER_zf22=[];
f_averge_all=[];
...
hError = comm.ErrorRate('ComputationDelay',0,'ReceiveDelay', 34);

%%% ofdm子载波正交，两天线发出的子载波正交(数据留出导频放置的位置)
% rng(22);
dain=randi([0 1],modDim.DataInputSize(1)/2,1);
dain2=randi([0 1],modDim.DataInputSize(1)/2,1);
DATAIN=[dain,dain2];

dain=step(hConEnc,dain);
dain=matintrlv(dain, JIA0_NUM, length(dain)/JIA0_NUM);
% OF_Data1=reshape(ANT1',1,[]);%%%%%%%%%%%% 生成数据帧

% rng(29);

dain2=step(hConEnc,dain2);
dain2=matintrlv(dain2, JIA0_NUM, length(dain2)/JIA0_NUM);
% ANT2=[nodain,dain2];
...
%% 1.2构造加PN序列的帧包
[PN,datain_ALL]=Canshu_MIMO(Npn,modData);%%%%datain_ALL完成的帧包
[PN,datain_ALL2]=Canshu_MIMO(Npn,modData2);%%%%datain_ALL完成的帧包
...
for lh=1:size(LTEChanOut,2)%%%%%%%%%%%大循环1.接受天线1；  2.接收天线2；
 %%%%%%加上空口时延
LTEChanOut=[zeros(addzero,2);LTEChanOut];
[Data_atertimelock,judg,Guard_atertimelock]=TimeLockFun_MIMO(PN,LTEChanOut(:,lh),length(modData));
 % 频偏估计
[f_averge,Data_atertFrelock]=frequencLock_MIMO(Data_atertimelock,Guard_atertimelock,Npn,K,Ts);
%数据重新存储
frequbias_temp=[frequbias_temp,abs(-f_averge-frequbias)./frequbias];
...
f_averge_all=[f_averge_all;frequbias_temp];
% figure()
% plot(1:length(Y),Y,'-P');
%% 最大似然法
Get_DATA=[];
...
for iwi=1:1:size(Get_DATA,2)
data_FINI = step(pskDemodulator,Get_DATA(:,iwi));%%%%%%%%%%%最终数据判决（BPSK解调）dataOut(估计前)ND_OUT（估计后）
data_FINI=matdeintrlv(data_FINI, JIA0_NUM, length(data_FINI)/JIA0_NUM);
% reset(hDec);
hDec = comm.ViterbiDecoder('InputFormat','Hard');
data_FINI=step(hDec, data_FINI);
% errorRate = comm.ErrorRate;
reset(hError);
        errVec = hError(DATAIN(:,iwi),data_FINI);   % data_FINI,dain;
%        errVec = errorRate(data_FINI,DATAIN(:,iwi));   % data_FINI,dain;
%         ber=[ber;errVec(1)];
BER_zui=[BER_zui,errVec(1)];
...
% errorRate = comm.ErrorRate;
reset(hError);
        errVec = hError(DATAIN(:,1),data_FINI);  
BER_zf11=[BER_zf11,errVec(1)];
...
% errorRate = comm.ErrorRate;
reset(hError);
        errVec = hError(DATAIN(:,2),data_FINI);  
BER_zf12=[BER_zf12,errVec(1)];
